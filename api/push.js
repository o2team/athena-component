...
module.exports = async (ctx, next) => {
  ...
  let widgetId = body.widgetId
	...
	await Promise.resolve().then(function() {
		...
		if(classify) {
			return new Promise(function(resolve, reject) {
				let query = new AV.Query('Classify')
    ...
    let widget = widgetId ? AV.Object.createWithoutData('Widget', widgetId) : new AV.Object('Widget')

		// https://leancloud.cn/docs/relation_guide-js.html#使用_Pointers_实现一对多关系
		if(business) {
			let bus = AV.Object.createWithoutData('Business', business)
			widget.set('business', bus)
		}
		if(classify) {
			let cls = AV.Object.createWithoutData('Classify', classify)
		...
		// 解压文件
		util.unzipWidget(w.id, true)
  		.then(function() {
  			setTimeout(function() {
          // 编译组件
    			util.buildWidget(w.id, w, true).catch(function(err) {
    				console.error(err)
    			})
  			}, 5000)
  		})
